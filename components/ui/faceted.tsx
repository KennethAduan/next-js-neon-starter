...
import { IconCheck, IconArrowsDownUp } from "@tabler/icons-react"
...
function useFacetedContext(name: string) {
  const context = React.use(FacetedContext)
  if (!context) {
...
function Faceted<TMultiple extends boolean = false>(
  ...
  const onOpenChange = React.useCallback(
    (newOpen: boolean, eventDetails?: unknown) => {
      ...
      if (onOpenChangeProp) {
        ;(onOpenChangeProp as (open: boolean, eventDetails?: unknown) => void)(
          newOpen,
          eventDetails
        )
      }
...
function FacetedBadgeList(props: FacetedBadgeListProps) {
  ...
  if (values.length === 0) {
    return (
      ...
      >
        {placeholder}
        <IconArrowsDownUp className="ml-auto size-4 shrink-0 opacity-50" />
      </div>
    )
  }
